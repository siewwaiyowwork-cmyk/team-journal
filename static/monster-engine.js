...
function mGetArchetype(s) {
  const dom = mGetDominant(s), sec = mGetSecondary(s), total = mGetTotal(s);
...
function drawMonster(svg, s, size) {
  ...
  const total = mGetTotal(s) / 500, bodyScale = 0.7 + total * 0.3;
  ...
  const bw = Math.round(size * 0.28 * bodyScale), bh = Math.round(size * 0.28 * bodyScale);
  ...
  if (s.atk > 20) {
    const hornH = size * (0.04 + s.atk/100 * 0.1), hornW = size * 0.04;
  ...
  if (s.agi > 40) {
    const wingW = size * (0.1 + s.agi/100 * 0.18), wingH = size * (0.08 + s.agi/100 * 0.1);
    const wy = by + bh*0.3;
    for(const side of [-1,1]) {
